...
def parse_args():
    ...
    parser.add_argument(
        '--bins', dest='bins', help='Number of angle bins (28 for mpiigaze, 90 for gaze360).',
        default=28, type=int)
    parser.add_argument(
        '--angle', dest='angle', help='Maximum gaze angle to include.',
        default=42, type=int)
    parser.add_argument(
        '--bin_width', dest='bin_width', help='Width of each angle bin in degrees.',
        default=3, type=int)
    # ---------------------------------------------------------------------------------------------------------------------
    # Important args ------------------------------------------------------------------------------------------------------
    ...
    if data_set=="gaze360":
        ...
        with open(os.path.join(evalpath,data_set+".log"), 'w') as outfile:
            ...
            for epochs in folder:
                ...
                saved_state_dict = torch.load(os.path.join(snapshot_path, epochs), weights_only=False)
        ...
        for fold in range(15):
            ...
            # list all epochs for testing — support both flat (fold0.pkl) and subdir (fold0/epoch.pkl) layouts
            fold_subdir = os.path.join(snapshot_path, "fold"+str(fold))
            if os.path.isdir(fold_subdir):
                epoch_files = sorted(os.listdir(fold_subdir), key=natural_keys)
                epoch_paths = [os.path.join(fold_subdir, e) for e in epoch_files]
            else:
                flat_pkl = os.path.join(snapshot_path, f"fold{fold}.pkl")
                epoch_files = [f"fold{fold}.pkl"]
                epoch_paths = [flat_pkl]
            ...
            with open(os.path.join(evalpath, os.path.join("fold"+str(fold), data_set+".log")), 'w') as outfile:
                ...
                epoch_list=[]
                avg_MAE=[]
                for epochs, epoch_path in zip(epoch_files, epoch_paths):
                    model=model_used
                    saved_state_dict = torch.load(epoch_path, weights_only=False)
