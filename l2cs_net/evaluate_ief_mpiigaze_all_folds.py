"""
Evaluate IEF head trained on top of native MPIIGaze backbone — all 15 folds.

Companion to train_ief_mpiigaze.py.
Checkpoints live in output/refinement_mpiigaze/best_foldN.pt
Results saved to  output/eval_ief_mpiigaze/

Usage:
  python evaluate_ief_mpiigaze_all_folds.py ^
    --model_dir  "models/MPIIGaze-20260529T070835Z-3-001/MPIIGaze" ^
    --ckpt_dir   "output/refinement_mpiigaze" ^
    --image_dir  "datasets/MPIIFaceGaze/Image" ^
    --label_dir  "datasets/MPIIFaceGaze/Label" ^
    --output     "output/eval_ief_mpiigaze"
"""
...
# MPIIGaze bin config — matches the backbone
BIN_WIDTH = 3.0
ANGLE_OFF = 42.0
NUM_BINS  = 28
...
def parse_args():
    ...
    p.add_argument("--ckpt_dir",   default="output/refinement_mpiigaze")
    p.add_argument("--image_dir",  required=True)
    p.add_argument("--label_dir",  required=True)
    p.add_argument("--output",     default="output/eval_ief_mpiigaze")
...
def main():
    ...
    for fold in range(15):
        ...
        all_final.append(fold_results[-1])
        print()
    ...
    if missing:
        print(f"\nNote: folds {missing} skipped (checkpoints not yet available).")

    # Save
    json_path = os.path.join(args.output, "ief_mpiigaze_all_folds.json")
    with open(json_path, "w") as f:
        json.dump({str(k): v for k, v in all_data.items()}, f, indent=2)

    txt_path = os.path.join(args.output, "ief_mpiigaze_all_folds.txt")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("IEF on MPIIGaze native backbone — all folds (final step)\n")
